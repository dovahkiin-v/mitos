...
- **It audits itself.** The corpus sweep (`mitos check -p .`) finds decisions that silently contradict each other. The commit gate (`mitos hook-install -p .`) runs on `git commit` and holds a commit made after decisions were recorded until a check has been attempted, and `--staged` checks hand-authored entries still pending in `decisions.md` — see [SETUP.md](SETUP.md) for the commit gate, the staged gate, and the CI and cron recipes.
