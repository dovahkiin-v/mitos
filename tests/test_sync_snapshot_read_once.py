...
(measured 2026-09-12: 47% of a 3,704-entry cold sync). The rotated blocks are still
sliced from that single read, so the happy-path rotation rows in `test_sync.py`
keep covering the content.
"""
...
def test_a_multi_entry_sync_reads_the_snapshot_once() -> None:
    ...
    try:
        ...
        # Two opens, independent of entry count: one to parse the snapshot, one to
        # slice raw blocks for rotation. The per-entry re-read made this 1 + 5.
        assert len(snapshot_reads) == 2, f"snapshot opened {len(snapshot_reads)} times"
    finally:
