...
instructions) so the agent uses Mitos correctly. `mitos status .` notes when that
file differs from what the installed mitos writes, and prints the command that
refreshes it.
...
| **Amend a committed entry's commentary (rejected paths, invalidates-if, context, scope, slug)** | `mitos amend-commentary` | `amend_commentary` |
...
| `amend_commentary`  | `mitos amend-commentary` | — |
...
> **To fix a committed entry's commentary without hand-editing it, use the tool.**
> `mitos amend-commentary -p . <slug> …` (the `amend_commentary` MCP tool) edits an
> entry's `rejected_paths`, `invalidates_if`, `context`, `scope` or slug in
> `decisions.md`, then re-commits it through the normal parse→commit path; the node id
> and its edges are unchanged. A change to the axiom or the mechanisms is refused: that
> is a new decision declaring `corrects` (it was wrong), `supersedes` (it was outgrown)
> or `amends` (it changed in part). It reaches entries still in `decisions.md`. Every
> entry stays there until the buffer holds `rotation_volume_threshold_entries`
> (default 50); after that, entries settled for longer than `rotation_lag_days`
> (default 14 days) move to `decisions/archive/`, so on a busy corpus the tool reaches
> roughly the last `rotation_lag_days` of writing. An archived entry answers
> `archived`: edit it in its archive file and run `mitos rebuild -p .`, which replays
> archives. A rename leaves entries whose relation lines cite the old slug refusing
